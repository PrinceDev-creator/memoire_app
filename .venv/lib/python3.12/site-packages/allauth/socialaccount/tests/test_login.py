...
from allauth.account.authentication import LOGIN_METHODSS_SESSION_KEY
...
def test_record_authentication(
    ...
    if did_record:
        assert request.session[LOGIN_METHODSS_SESSION_KEY] == [
            {
                "at": ANY,
        ...
        assert LOGIN_METHODSS_SESSION_KEY not in request.session
    assert (
