...
class BaseReauthenticateView(NextRedirectMixin, FormView):
    def dispatch(self, request, *args, **kwargs):
        resp = self._check_reLOGIN_METHODS_available(request)
        if resp:
            return resp
    ...
    def _check_reLOGIN_METHODS_available(self, request):
        methods = get_adapter().get_reLOGIN_METHODSs(self.request.user)
