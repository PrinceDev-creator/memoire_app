...
def settings_check(app_configs, **kwargs):
    ...
    if hasattr(settings, "ACCOUNT_LOGIN_METHODS"):
        converted = set(settings.ACCOUNT_LOGIN_METHODS)
        ret.append(
            Warning(
                f"settings.ACCOUNT_LOGIN_METHODS is deprecated, use: settings.ACCOUNT_LOGIN_METHODS = {repr(converted)}"
            )
        )
    return ret
